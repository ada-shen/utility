...
from models import RSCNN_DWS_Cls 
...
import random
import yaml
from provider import random_rotate_batchdata
...
parser.add_argument('--config', default='cfgs/config_dws_cls.yaml', type=str)
...
def main():
    ...
    train_dataset = DataSetCls(num_points = args.num_points, root = args.data_root, transforms=train_transforms)
    ...
    model = RSCNN_DWS_Cls(num_classes = args.num_classes, input_channels = args.input_channels, relation_prior = args.relation_prior, use_xyz = True, block_choice=args.block_choice)
    model.cuda()
...
def train(train_dataloader, test_dataloader, model, criterion, optimizer, lr_scheduler, bnm_scheduler, args, num_batch):
    ...
    global g_acc 
    g_acc = 0.81    # only save the model whose acc > 0.91
    batch_count = 0
    model.train()
    for epoch in range(args.epochs):
        for i, data in enumerate(train_dataloader, 0):
            ...
            points, target = data
            points = torch.from_numpy(random_rotate_batchdata(points)).float() ####random rotate the input pointcloud
...
def validate(test_dataloader, model, criterion, args, iter): 
    ...
    acc = (preds == labels).sum().detach().cpu().numpy() / labels.numel()

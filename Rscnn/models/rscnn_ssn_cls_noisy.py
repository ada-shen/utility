...
class RSCNN_SSN_NOISY(nn.Module):
    r"""
...
def get_similarity_loss(mid_feature , sigma, sigmaf,  sigma_weight = 0.0028):
    batch_size = int(mid_feature.size()[0]/2)
    real_feature = mid_feature[:batch_size,:]
    jitter_feature = mid_feature[batch_size:,:]
    feature_loss = torch.sum((jitter_feature-real_feature)**2)/batch_size
    if sigmaf is None:
        point_weight = 1.0
    else:
        point_weight = 1.0 / sigmaf
    sigma_loss = torch.sum(torch.log(torch.abs(sigma)))/batch_size

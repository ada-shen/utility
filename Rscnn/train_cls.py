...
from models import RSCNN_SSN_Cls 
from models import RSCNN_MSN_Cls
...
def main():
    ...
    if args.model_name == 'rscnn':
        model = RSCNN_SSN_Cls(num_classes = args.num_classes, input_channels = args.input_channels, relation_prior = args.relation_prior, use_xyz = True)
    elif args.model_name == 'rscnn_msn':
        model = RSCNN_MSN_Cls(num_classes = args.num_classes, input_channels = args.input_channels, relation_prior = args.relation_prior, use_xyz = True)
...
def train(train_dataloader, test_dataloader, model, criterion, optimizer, lr_scheduler, bnm_scheduler, args, num_batch):
    ...
    global g_acc 
    g_acc = 0.85    # only save the model whose acc > 0.91
    batch_count = 0
    model.train()

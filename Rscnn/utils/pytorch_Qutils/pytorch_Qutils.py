...
class QBN_RM(nn.Module):
    ...
    def forward(self, input):
        ...
        # coefficient = self.running_average.view(1,inSize[1],1,1).cuda()
        coefficient = mod_xd2.view(1,inSize[1],1,1).cuda()
...
class RSConv(nn.Module):
    ...
    def __init__(
        ...
        self.bn_rsconv = QBN_RM(C_in) if not first_layer else QBN_RM(16)
        self.bn_channel_raising = QBN_RM1d(C_out)
        self.bn_xyz_raising = QBN_RM(16)
        if first_layer:
        ...
        self.mapping_func3 = mapping[2]
        self.cr_mapping = mapping[3]
        if first_layer:
            self.xyz_raising = mapping[4]
        self.Qmaxpool = our_mpool1d.apply

    def forward(self, input):
        #input: (B,  3+3, npoint, centroid + nsample)
        #feature: (B*3, C_in, npoint, centroid + nsample)
        xyz, feature = input
        B, C, N, S = xyz.shape
        if self.relation_prior == 2:
            ...
            abs_coord = xyz[:, 0:3, :, :]  # (B, 3, npoint, nsample+1), absolute coordinates
            delta_x = xyz[:, 3:6, :, :]    # (B, 3, npoint, nsample+1), normalized coordinates
        
        h_xi_xj = torch.norm(delta_x, p = 2, dim = 1).unsqueeze(1)
        del abs_coord, delta_x 

        h_xi_xj = self.mapping_func2(self.activation(self.bn_mapping(self.mapping_func1(h_xi_xj))))

        if self.first_layer:
            feature = Qrelu(self.bn_xyz_raising(self.xyz_raising(feature)))

        h_C = h_xi_xj.shape[1]
        h_xi_xj = h_xi_xj.view(B,1,h_C,N,S).repeat(1,3,1,1,1).view(B*3,h_C,N,S)
        
        #ori#feature =  self.Qmaxpool(Qrelu(self.bn_rsconv(torch.mul(h_xi_xj, feature))), (1,S), (1,S))
        #有提升feature =  self.Qmaxpool(torch.mul(h_xi_xj, feature), (1,S), (1,S))
        #加一层卷积
        feature =  self.Qmaxpool(Qrelu(self.bn_rsconv(self.mapping_func3(torch.mul(h_xi_xj, feature)))), (1,S), (1,S))
        del h_xi_xj
        
        feature = Qrelu1d(self.bn_channel_raising(self.cr_mapping(feature)))
        
        # feature = Qrelu(self.bn_rsconv(torch.mul(h_xi_xj, feature)))
        # feature = Qrelu(self.bn_channel_raising(self.cr_mapping(feature)))
        # feature = self.Qmaxpool(feature,(1,S), (1,S))
...
class GloAvgConv(nn.Module):
    ...
    def __init__(
        ...
        self.bn_avg = QBN_RM(C_out)
        init(self.conv_avg.weight)
        self.Qmaxpool = our_mpool1d.apply
        
    def forward(self, x):
        nsample = x.size()[3]
        x = Qrelu(self.bn_avg(self.conv_avg(x)))
        
        x = self.Qmaxpool (x,  (1, nsample),  (1, nsample))
        return x

...
import pytorch_Qutils as pt_utils
...
class _PointnetSAModuleBase(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor,
        ...
        for i in range(len(self.groupers)):
            if self.npoint is not None:
                group_xyz, new_features = self.groupers[i](xyz, new_xyz, features, fps_idx) 
                new_features = (self.mlps[i])((group_xyz,new_features))  # (B, mlp[-1], npoint) 
            else:
...
class PointnetSAModuleMSG(_PointnetSAModuleBase):
    ...
    def __init__(
        ...
        C_in = (mlps[0][0] + 2) if use_xyz else mlps[0][0]
        C_out = mlps[0][1]
        ...
        if first_layer:
            ...
            mapping_func2 = nn.Conv2d(in_channels = math.floor(C_out / 2), out_channels = 16, kernel_size = (1, 1), 
                                  stride = (1, 1), bias = bias)
            mapping_func3 = nn.Conv2d(in_channels = 16, out_channels = 16, kernel_size = (1, 1), 
            ...
            mapping_func2 = nn.Conv2d(in_channels = math.floor(C_out / 4), out_channels = C_in , kernel_size = (1, 1), 
                                stride = (1, 1), bias = bias)
            mapping_func3 = nn.Conv2d(in_channels = C_in, out_channels = C_in , kernel_size = (1, 1), 
                                stride = (1, 1), bias = False)
        if npoint is not None:
            init(mapping_func1.weight)
            init(mapping_func2.weight)
            init(mapping_func3.weight)
            if bias:
        ...
        if first_layer:
            mapping = [mapping_func1, mapping_func2, mapping_func3, cr_mapping, xyz_raising]
        elif npoint is not None:
            mapping = [mapping_func1, mapping_func2, mapping_func3, cr_mapping]
        
        for i in range(len(radii)):
            radius = radii[i]
            nsample = nsamples[i]
            self.groupers.append(
                pointnet2_utils.QueryAndGroupQ(radius, nsample, use_xyz=use_xyz)
                if npoint is not None else pointnet2_utils.GroupAllQ(use_xyz)
            )
            mlp_spec = mlps[i]
            if use_xyz:
                mlp_spec[0] += 2
            if npoint is not None:
                ...
                self.mlps.append(pt_utils.GloAvgConv(C_in = C_in-1, C_out = C_out))

...
class _PointnetSAModuleBase(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor,
        ...
        if self.npoint is not None:
            batch_size = int(xyz.shape[0]/2)
            fps_idx = pointnet2_utils.furthest_point_sample(xyz[:batch_size,...], self.npoint)  # (B, npoint)
            new_xyz = pointnet2_utils.gather_operation(xyz[:batch_size,...].transpose(1, 2).contiguous(), fps_idx).transpose(1, 2).contiguous()
            center_xyz = pointnet2_utils.gather_operation(xyz_flipped, fps_idx.repeat(2,1)).transpose(1, 2).contiguous()
            new_xyz = new_xyz.repeat(2,1,1)
            # print(new_xyz.shape)
            # print(xyz.shape)
            
            fps_idx = fps_idx.repeat(2,1).data
            # print(fps_idx.shape)
        else:
            new_xyz = None
            center_xyz = None
        ...
        for i in range(len(self.groupers)):
            new_features, grouped_xyz_norm, idx = self.groupers[i](xyz, new_xyz, features, fps_idx) if self.npoint is not None else self.groupers[i](xyz, new_xyz, features)  # (B, C, npoint, nsample)
            new_features = self.mlps[i](
                [new_features, xyz, grouped_xyz_norm, idx]
            )  # (B, mlp[-1], npoint)

            new_features_list.append(new_features)
        
        return center_xyz, torch.cat(new_features_list, dim=1)
    ...
    def __init__(
        ...
        for i in range(len(radii)):
            ...
            if use_xyz:
                mlp_spec[0] += 3
            if npoint is not None:
                self.mlps.append(pt_utils.SharedRSConv(mlp_spec, mapping = mapping, relation_prior = relation_prior, first_layer = first_layer, block_choice=block_choice))
            else:   # global convolutional pooling
                self.mlps.append(pt_utils.GloAvgConv(C_in = C_in, C_out = C_out, block_choice=block_choice))

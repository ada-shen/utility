...
import math
from conv_sift import PointConvWeight, PointSIFT_res_module, PointConvDensity
...
class RSConv(nn.Module):
    ...
    def __init__(
        ...
        if block_choice=='weight':
            self.WeightNet = PointConvWeight()
        elif block_choice=='density':
            self.DensityNet = PointConvDensity()
        elif block_choice=='sift':
            self.SiftNet1 = PointSIFT_res_module(radius=0.25, output_channel=128, extra_input_channel=128)
            self.SiftNet2 = PointSIFT_res_module(radius=0.5, output_channel=512, extra_input_channel=512)
        if first_layer:
            self.xyz_raising = mapping[3]
        
    def forward(self, input_set): # input: (B, 3 + 3 + C_in, npoint, centroid + nsample)
        input = input_set[0]
        raw_xyz = input_set[1]
        grouped_xyz_norm = input_set[2]
        idx = input_set[3]
        #####
        ...
        x = self.activation(self.bn_rsconv(torch.mul(h_xi_xj, x)))
        ####### DWS Block###########
        if self.block_choice == 'weight':
            x = self.WeightNet(input[:, 0:3, :, :].cuda(), x.cuda())
            nsample = x.size()[3]
        elif self.block_choice == 'density':
            x = self.DensityNet(raw_xyz, grouped_xyz_norm, idx, x)
            nsample = x.size()[3]
        ...
        ####### DWS Block:Sift###########
        # print('shape:', raw_xyz.shape, input.shape, x.shape, grouped_xyz_norm.shape, idx.shape)
        if self.block_choice == 'sift':
            if not self.first_layer:
                _, x = self.SiftNet2(input[:, 0:3, :, 0], x)
            # else:
            #     _, x = self.SiftNet2(input[:, 0:3, :, 0],x) 
        #########################################
        return x
...
class GloAvgConv(nn.Module):
    ...
    def __init__(
        ...
        self.activation = activation
        self.block_choice = block_choice
        self.SiftNet = PointSIFT_res_module(radius=0.5, output_channel=256, extra_input_channel=256)
    ...
    def forward(self, input_set):
        ...
        nsample = x.size()[3]
        # if self.block_choice == 'weight':
        #     x = self.WeightNet(input[:, 0:3, :, :], x)
        # elif self.block_choice == 'density':
        #     c = F.max_pool2d(x)
        # elif self.block_choice == 'sift':
        #     pass

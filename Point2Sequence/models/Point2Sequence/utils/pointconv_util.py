...
def sampling(npoint, ori_pts, sa_pts):
    '''
    inputs:
    ...
    '''
    idx = tf_sampling.farthest_point_sample(npoint, sa_pts)
    idx = tf.concat((idx,idx), axis=0)
    center_xyz = tf_sampling.gather_point(ori_pts,idx)
    sa_pts = tf.concat((sa_pts,sa_pts), axis=0)
    new_xyz = tf_sampling.gather_point(sa_pts,idx)
    #sub_pts = tf_sampling.gather_point(pts, tf_sampling.farthest_point_sample(npoint, pts))
    return center_xyz, new_xyz

def grouping(feature, K, src_xyz, q_xyz, sample_xyz = None, center_xyz =None, use_xyz = True):
    '''
    K: neighbor size
    ...
    point_indices = tf.py_func(knn_kdtree, [K, sample_xyz, q_xyz], tf.int32)
    ...
    grouped_xyz -= tf.tile(tf.expand_dims(center_xyz, 2), [1,1,K,1]) # translation normalization

...
from noisy import perturbation_point_xyz, perturbation_point_uniform
...
def get_model(point_cloud, is_training, num_class, bn_decay=None, feature=None):
    ...
    end_points = {}
    point_cloud, pert = perturbation_point_xyz(point_cloud)
    l0_xyz = point_cloud
    l0_points = feature
    ...
    # c3
    c0_l3_xyz, c0_l3_points, c0_l3_indices = pointSIFT_res_module(l3_xyz, l3_points, radius=0.85, out_channel=512, is_training=is_training, bn_decay=bn_decay, scope='layer3_c0')
...
def get_adversarial_loss(pred, label, pert, lam = 1.0):
    #predict = tf.argmax(pred, axis=-1)
    #acc = tf.reduce_sum(tf.cast(tf.equal(predict, tf.to_int64(label)), dtype='int32'))
    #acc = tf.to_float(acc)/pred.get_shape()[0].value
    pred = tf.reshape(pred, [1,40])
    # print(pred.shape)
    # print(label.shape)
    ...
    pert_loss = tf.square(tf.norm(pert, ord = 'euclidean'))#2*tf.nn.l2_loss(pert)
    total_loss = classify_loss_mean + lam * pert_loss
    tf.summary.scalar('classify loss', classify_loss_mean)
    tf.summary.scalar('total loss', total_loss)
    return total_loss, classify_loss_mean, pert_loss

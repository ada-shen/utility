...
sys.path.append(os.path.join(ROOT_DIR, 'tf_utils'))
sys.path.append(os.path.join(ROOT_DIR, 'tf_utils/tf_ops/sampling'))
sys.path.append(os.path.join(ROOT_DIR, 'tf_utils/tf_ops/grouping'))
sys.path.append(os.path.join(ROOT_DIR, 'tf_utils/tf_ops/interpolation'))
sys.path.append(os.path.join(ROOT_DIR, 'tf_utils/tf_ops/pointSIFT_op'))
from pointSIFT_op import pointSIFT_select, pointSIFT_select_four
from tf_grouping import group_point, query_ball_point, knn_point
...
def pointSIFT_group_noise(radius, xyz, points, use_xyz=True):
    ori_size = xyz.get_shape()[0].value // 2
    sample_xyz = tf.gather(xyz,[i for i in range(ori_size)])
    sample_xyz = tf.concat((sample_xyz,sample_xyz),axis=0)

    idx = pointSIFT_select(sample_xyz, radius)
...
def pointSIFT_res_module_noise(xyz, points, radius, out_channel, is_training, bn_decay, scope='point_sift', bn=True, use_xyz=True, same_dim=False, merge='add'):
    data_format = 'NHWC'
    with tf.variable_scope(scope) as sc:
        # conv1
        _, new_points, idx, _ = pointSIFT_group_noise(radius, xyz, points, use_xyz=use_xyz)
...
def pointSIFT_nores_module_noise(xyz, points, radius, out_channel, is_training, bn_decay, scope='point_sift', bn=True, use_xyz=True, same_dim=False, merge='add'):
    data_format = 'NHWC'
    with tf.variable_scope(scope) as sc:
        # conv1
        _, new_points, idx, _ = pointSIFT_group_noise(radius, xyz, points, use_xyz=use_xyz)
...
def pointSIFT_nores_module(xyz, points, radius, out_channel, is_training, bn_decay, scope='point_sift', bn=True, use_xyz=True, same_dim=False, merge='add'):
    data_format = 'NHWC'
    with tf.variable_scope(scope) as sc:
        # conv1
...
def sample_and_group_noise(npoint, radius, nsample, xyz, points, knn=False, use_xyz=True):
    '''
    Input:
        npoint: int32
        radius: float32
    ...
    sample_idx = farthest_point_sample(npoint, sample_xyz)
    new_xyz = gather_point(sample_xyz, sample_idx ) # (batch_size, npoint, 3)

    center_xyz = gather_point(xyz, tf.concat((sample_idx,sample_idx),axis=0))

    new_xyz = tf.concat((new_xyz,new_xyz),axis=0)
    sample_xyz = tf.concat((sample_xyz,sample_xyz),axis=0)
    #new_xyz = gather_point(xyz, farthest_point_sample(npoint, xyz))  # (batch_size, npoint, 3)
    if knn:
        _, idx = knn_point(nsample, sample_xyz, new_xyz)
    else:
        idx, pts_cnt = query_ball_point(radius, nsample, sample_xyz, new_xyz)
    grouped_xyz = group_point(xyz, idx)  # (batch_size, npoint, nsample, 3)
    grouped_xyz -= tf.tile(tf.expand_dims(center_xyz, 2), [1, 1, nsample, 1])  # translation normalization
    if points is not None:
        grouped_points = group_point(points, idx)  # (batch_size, npoint, nsample, channel)
        if use_xyz:
            new_points = tf.concat([grouped_xyz, grouped_points], axis=-1)  # (batch_size, npoint, nample, 3+channel)
    ...
    return center_xyz, new_points, idx, grouped_xyz
...
def pointnet_sa_module_noise(xyz, points, npoint, radius, nsample, mlp, mlp2, group_all, is_training, bn_decay, scope,
                       bn=True, pooling='max', knn=False, use_xyz=True, use_nchw=False):
    ''' PointNet Set Abstraction (SA) Module
        Input:
            xyz: (batch_size, ndataset, 3) TF tensor
    ...
    with tf.variable_scope(scope) as sc:
        ...
        if group_all:
            ...
            new_xyz, new_points, idx, grouped_xyz = sample_and_group_noise(npoint, radius, nsample, xyz, points, knn, use_xyz)
        ...
        for i, num_out_channel in enumerate(mlp):
            new_points = tf_util.conv2d(new_points, num_out_channel, [1, 1],
                                        ...
                                        scope='conv%d' % (i), bn_decay=bn_decay,
                                        data_format=data_format)
            # end_points[scope+'_'+str(i)] = new_points
        if use_nchw: new_points = tf.transpose(new_points, [0, 2, 3, 1])
...
def point2sequence_oe_noise(xyz, points, npoint, nsample_list, mlp_list, hidden_size, output_size, is_training, bn_decay, scope, bn=True, use_xyz=True, use_nchw=False, batch_size=16):
    with tf.variable_scope(scope) as sc:

        sample_xyz = tf.gather(xyz,[i for i in range(xyz.get_shape()[0].value // 2)])
        sample_idx = farthest_point_sample(npoint, sample_xyz)
        new_xyz = gather_point(sample_xyz, sample_idx) # (batch_size, npoint, 3)
        ...
        for i in range(len(nsample_list)):
            ###add OE layer###
            _, sift_points,_ = pointSIFT_nores_module_noise(xyz,points,radius=0.2,out_channel=mlp_list[i][0],is_training=is_training, bn_decay=bn_decay, scope='SIFTOE_%d'%i,merge='concat')
             
            nsample = nsample_list[i]
            vla, idx = knn_point(nsample, sample_xyz, new_xyz)
            grouped_xyz = group_point(xyz, idx)

            # convert to the relative coordinate system
            grouped_xyz -= tf.tile(tf.expand_dims(center_xyz, 2), [1, 1, nsample, 1])
            if sift_points is not None:
                grouped_points = group_point(sift_points, idx)
                if use_xyz:
                    grouped_points = tf.concat([grouped_points, grouped_xyz], axis=-1)
            else:
                grouped_points = grouped_xyz
        ...
        return center_xyz, feature#, midfeatureone, midfeaturetwo

def point2sequence_module(xyz, points, npoint, nsample_list, mlp_list, hidden_size, output_size, is_training, bn_decay, scope, bn=True, use_xyz=True, use_nchw=False, add_oe=False):
    with tf.variable_scope(scope) as sc:
        new_xyz = gather_point(xyz, farthest_point_sample(npoint, xyz))
        new_points_list = []
        for i in range(len(nsample_list)):
            ###add OE layer###
            if add_oe:
                _, sift_points, _ = pointSIFT_nores_module(xyz, points, radius=0.2, out_channel=mlp_list[i][0], is_training=is_training, bn_decay=bn_decay, scope='SIFTOE_%d'%i, merge='concat')
            else:
                sift_points = points
            nsample = nsample_list[i]
...
def point2sequence_module_noise(xyz, points, npoint, nsample_list, mlp_list, hidden_size, output_size, is_training, bn_decay, scope, bn=True, use_xyz=True, use_nchw=False, batch_size=16):
    with tf.variable_scope(scope) as sc:

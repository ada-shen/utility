...
def pointnet_sa_module(xyz, points, npoint, radius, nsample, mlp, mlp2, group_all, is_training, bn_decay, scope, bn=True, pooling='max', knn=False, use_xyz=True, use_nchw=False):
    ...
    with tf.variable_scope(scope) as sc:
        ...
        #####################interpretable######################
        #global_pre_feature = new_points
        #global_pre_index = tf.argmax(global_pre_feature,axis=2)
        #class_index = tf.argmax(global_pre_feature,axis=3)
        #########################################################
...
def pointnet_sa_module_density(xyz, points, npoint, radius, sigma, nsample, mlp, mlp2, group_all, is_training, bn_decay, scope, sigma_init, bn=True, pooling='max', knn=False, use_xyz=True, use_nchw=False):
    ''' PointNet Set Abstraction (SA) Module
        Input:
            xyz: (batch_size, ndatasetdef max_point_index(global_pre_feature, batch_size, num_point):
    global_pre_feature = tf.reshape(global_pre_feature,[batch_size, num_point, 16])
...
def pointnet_sa_module_weight(xyz, points, npoint, radius, nsample, mlp, mlp2, group_all, is_training, bn_decay, scope,sigma_init,  bn=True, pooling='max', knn=False, use_xyz=True, use_nchw=False):
    ''' PointNet Set Abstraction (SA) Module
        Input:
            xyz: (batch_size, ndatasetdef max_point_index(global_pre_feature, batch_size, num_point):
    global_pre_feature = tf.reshape(global_pre_feature,[batch_size, num_point, 16])
    ...
    with tf.variable_scope(scope) as sc:
        ...
        if group_all:
            ...
            if sigma_init is None:
                ...
                print("Using the fixsample model")
                new_xyz, new_points, idx, grouped_xyz = sample_and_group_noise(npoint, radius, nsample, xyz, points, knn, use_xyz)
